...
func createSQLFiles(db migrations.DB, filename string, isTransaction bool) error {
	...
	fileNameDown := ""
	if isTransaction {
		fileNameUp = fmt.Sprintf("%d_%s.tx.up.sql", vers+1, filename)
		fileNameDown = fmt.Sprintf("%d_%s.tx.down.sql", vers+1, filename)
	} else {
		fileNameUp = fmt.Sprintf("%d_%s.up.sql", vers+1, filename)
		fileNameDown = fmt.Sprintf("%d_%s.down.sql", vers+1, filename)
	}
