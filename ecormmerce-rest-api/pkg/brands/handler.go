...
func (h *Handlers) handleAddBrand(response http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		brandHandlerLogging.Printlog("Brand HandleAddBrand; Error while decoding request body:", err.Error())
	...
	format.Send(response, http.StatusOK, format.Message(true, "Brand saved", nil))
...
func (h *Handlers) handleUpdateBrand(response http.ResponseWriter, request *http.Request) {
	fmt.Println("update Brand")
	brand := Brand{}

	err := json.NewDecoder(request.Body).Decode(&brand)
	if err != nil {
		brandHandlerLogging.Printlog("Brand HandleUpdateBrand; Error while decoding request body:", err.Error())
		format.Send(response, http.StatusInternalServerError, format.Message(false, "Error while decoding request body", nil))
		return
	}
	...
	format.Send(response, http.StatusOK, format.Message(true, "Brand updated", nil))
...
func (h *Handlers) handleDeleteBrand(response http.ResponseWriter, request *http.Request) {
	...
	if !status {
		brandHandlerLogging.Printlog("Brand HandleUpdateBrand; Error getting brand id:", "Could not get id")
	...
	if err != nil {
		brandHandlerLogging.Printlog("Brand HandleUpdateBrand; Error while converting string to int:", err.Error())
		format.Send(response, http.StatusInternalServerError, format.Message(false, "Error occured while converting string to int", nil))
		return
	}
	brand.ID = id
	...
	format.Send(response, http.StatusOK, format.Message(true, "Brand deleted", nil))
...
func (h *Handlers) handleGetBrands(response http.ResponseWriter, request *http.Request) {
	...
	format.Send(response, http.StatusOK, format.Message(true, "All brands", brands)) // respond(response, message(true, "Brand saved"))
